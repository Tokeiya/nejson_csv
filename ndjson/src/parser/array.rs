...
fn contents<I: Stream<Token = char>, L: Logger>(
	...
	let content = (first::<I, L>(logger.clone()), following(logger.clone())).map(|(a, b)| {
		let mut vec = b;
		vec.insert(0, a);
...
pub fn array<I: Stream<Token = char>, L: Logger>(
	logger: Rc<RefCell<L>>,
) -> impl Parser<I, Output = NodeValue> {
	(chr::char('['), contents(logger.clone()), chr::char(']')).map(|(_, v, _)| v)
}
...
mod test {
	...
	fn array() {
		...
		let mut parser = super::array::<&str, _>(gen_logger());
