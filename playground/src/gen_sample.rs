...
fn generate_object(
	...
	for (idx, key) in keys.iter().enumerate() {
		writer.write_fmt(format_args!(r#"{str}"{key}":"#))?;
		generate_node(
			writer,
			rnd,
			cnt,
...
fn generate_node(
	...
	if current_depth >= threshold_depth {
		...
		match choice_node(rnd) {
			NodeTypes::Array => {
				generate_array(writer, rnd, cnt, current_depth + 1, threshold_depth, recent)
			}
			NodeTypes::Object => {
				generate_object(writer, rnd, cnt, current_depth + 1, threshold_depth, recent)
			}
			NodeTypes::Terminal => {
				writer.write_fmt(format_args!(
					"{}{}",
...
pub fn generate_sample(writer: &mut impl Write, depth: usize) -> IoResult<()> {
	...
	generate_node(writer, &mut rnd, &Cell::new(0), 0, depth, RecentNode::Root)?;
